...
(function(){
  ...
  var scene,camera,renderer,clock,raycaster,pointer,snow,roof,route,warmLight,stableLight;
  var initialized=false,running=false,started=false,storming=false,finished=false;
  var assembled=new Set(),partGroups={},targets={},exploded={},cameraGoal;
  ...
  function buildHouse(){
    ...
    var path=new THREE.CatmullRomCurve3([new THREE.Vector3(-4.1,1.15,.55),new THREE.Vector3(-2.2,1.15,.55),new THREE.Vector3(-.5,1.05,.55),new THREE.Vector3(1.6,1.05,.55),new THREE.Vector3(3.7,1.05,.55),new THREE.Vector3(3.7,3.25,.4)]);
    route=new THREE.Mesh(new THREE.TubeGeometry(path,70,.065,8,false),new THREE.MeshBasicMaterial({color:0xffcb5a,transparent:true,opacity:.96,depthTest:false}));route.visible=false;route.renderOrder=4;scene.add(route);
  ...
  function explode(){
    started=true;finished=false;storming=false;assembled.clear();route.visible=false;warmLight.intensity=stableLight.intensity=0;roof.children.forEach(function(n){n.material.opacity=.94;});roof.position.y=3.2;
    ...
    startButton.hidden=true;stormButton.hidden=true;partsBox.hidden=false;reveal.hidden=true;stageCopy.textContent='Сборка · выбери объём';progressCopy.textContent='0 / 4';question.textContent=prompts[0];feedback.textContent='Нажимай на объёмы в 3D или на подписанные кнопки. Неверный выбор замёрзнет, но не остановит сборку.';setGoal(13,8.6,15);
  ...
  function selectPart(id){
    ...
    if(id!==expected){
      var wrongButton=partsBox.querySelector('[data-house-part="'+id+'"]');wrongButton.classList.add('is-wrong');g.scale.set(1.06,.94,1.06);feedback.textContent=wrongHints[assembled.size];setTimeout(function(){wrongButton.classList.remove('is-wrong');g.scale.set(1,1,1);},650);if(window.rcGoal)window.rcGoal('rc_game_attempt',{part:id,correct:false,step:assembled.size+1});return;
    }
    assembled.add(id);g.position.copy(targets[id]);g.rotation.set(0,0,0);
    var button=partsBox.querySelector('[data-house-part="'+id+'"]');button.disabled=true;button.classList.add('is-done');
    progressCopy.textContent=assembled.size+' / 4';feedback.innerHTML='<b>'+names[id]+'.</b> '+meanings[id];
    if(assembled.size===4){roof.position.y=0;partsBox.hidden=true;stormButton.hidden=false;stageCopy.textContent='Испытание · дом собран';question.textContent='Все части под одной кровлей. Выдержит ли маршрут метель?';feedback.textContent='Запусти непогоду и проследи путь от печи к животным и сену.';stormButton.focus();}else question.textContent=prompts[assembled.size];
  ...
  function finish(){
    if(assembled.size<4)return;storming=true;stormButton.hidden=true;stageCopy.textContent='Метель · проверка маршрута';question.textContent='Дом закрыл жизнь внутри единого тёплого контура';feedback.textContent='Крыша становится прозрачной. Золотая линия показывает путь: печь → сени → двор → животные → сеновал.';
    route.visible=true;warmLight.intensity=3.2;stableLight.intensity=2.4;roof.children.forEach(function(n){n.material.opacity=.2;});setPressedView('route');canvas.scrollIntoView({behavior:reduce?'auto':'smooth',block:'center'});
    if(reduce){setGoal(8,5.5,8);completeReveal();return;}
    setGoal(10,5.8,10);setTimeout(function(){setGoal(.3,2.1,1.25);},650);setTimeout(function(){setGoal(8,5.5,8);},2100);setTimeout(completeReveal,2700);
  }
  ...
  function reset(){
    started=storming=finished=false;assembled.clear();route.visible=false;warmLight.intensity=stableLight.intensity=0;roof.position.y=0;roof.children.forEach(function(n){n.material.opacity=.94;});
    Object.keys(partGroups).forEach(function(k){partGroups[k].position.copy(targets[k]);partGroups[k].rotation.set(0,0,0);});
    startButton.hidden=false;stormButton.hidden=true;partsBox.hidden=true;reveal.hidden=true;stageCopy.textContent='Наблюдение 1 из 4';progressCopy.textContent='0 / 4';question.textContent='Как огромный дом защищал повседневную жизнь от непогоды?';feedback.textContent='Сначала раскрой дом: кровля поднимется, а объёмы разойдутся в учебный разрез.';setPressedView('exterior');cameraGoal.copy(exteriorPosition());
  }
