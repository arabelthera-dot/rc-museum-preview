async (page) => {
 const root='/home/agent/dna/reports/ethnography-april-2026-09-09';
 await page.goto('http://172.22.0.4:8841/index.html');
 ...
 await page.getByRole('searchbox').fill('Степан Хомутин:');
 ...
 await page.setViewportSize({width:1440,height:1000});
 await page.pdf({path:root+'/april.pdf',format:'A3',landscape:true,printBackground:true,preferCSSPageSize:true});
 const result={initial,search,calendar,conditional,empty,mobile,errors};
 if(initial.rows!==30||initial.columns!==5||search!==1||calendar!==3||conditional!==27||!empty||initial.overflow||mobile.some(x=>x.overflow)||errors.length)throw new Error(JSON.stringify(result));
 return result;
}
