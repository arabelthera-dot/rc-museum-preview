...
(()=>{'use strict';document.querySelectorAll('[data-photo-order]').forEach(root=>{
 ...
 function paint(){host.replaceChildren();order.forEach((part,pos)=>{
  ...
  visual.className='order-fragment';img.src=root.dataset.image;img.alt=['Верх: край крышки и пустое отделение справа','Середина: два ряда небольших ящиков','Низ: высокие ящики и дно ларца'][part];img.style.top=(-part*100)+'%';visual.append(img);item.append(visual);
